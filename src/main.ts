...
import { EfficiencyLabScreen } from "./efficiency-lab/EfficiencyLabScreen.js";
import { StringManager } from "./i18n/StringManager.js";
import { IntroScreen } from "./intro/IntroScreen.js";
import { CarnotHeatEnginePreferencesModel } from "./preferences/CarnotHeatEnginePreferencesModel.js";
import { CarnotHeatEnginePreferencesNode } from "./preferences/CarnotHeatEnginePreferencesNode.js";
import { ReversedCycleScreen } from "./reversed-cycle/ReversedCycleScreen.js";

onReadyToLaunch(() => {
  const stringManager = StringManager.getInstance();

  // Simulation-specific preferences; initial values come from carnotHeatEngineQueryParameters.
  // Constructed once here and handed to every Screen, because the diagrams honour
  // the "label cycle corners" toggle.
  const simPreferences = new CarnotHeatEnginePreferencesModel(Tandem.ROOT.createTandem("preferences"));
  ...
  const sim = new Sim(stringManager.getTitleStringProperty(), screens, {
    preferencesModel: new PreferencesModel({
      ...
      simulationOptions: {
        customPreferences: [
          {
            createContent: (tandem: Tandem) => new CarnotHeatEnginePreferencesNode(simPreferences, tandem),
          },
        ],
      },
      localizationOptions: {
